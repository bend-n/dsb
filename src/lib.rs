    ...
    for (col, k) in cells.chunks_exact(c as _).zip(0..) {
        for (&cell, j) in zip(col, 0..) {
            if cell.style.bg != bgcolor {
                let cell = Image::<_, 4>::build(fw.ceil() as u32, fh_.floor() as u32)
            ...
            if let Some(_) = cell.letter {
                ...
                unsafe {
                    ...
                    i.as_mut().blend_alpha_and_color_at(
                        ...
                        (((k + 1) as f32 * (fh)) - (x.placement.top as f32)).round() as u32
...
// [a, \n, b, \n, c, \n, d]
pub fn fit(font: &FontRef, ppem: f32, line_spacing: f32, sizes: (usize, usize)) -> (usize, usize) {
    let m = font.metrics(&[]);
    ...
    (
        ...
        successors(Some(fh), |&x| {
            ((x + fh + line_spacing * f) < sizes.1 as f32).then_some(x + fh + (line_spacing * f))
        })
        .count(),
    )
}

fn height(font: &FontRef, ppem: f32, line_spacing: f32, rows: usize) -> usize {
    let m = font.metrics(&[]);
    let f = ppem / m.units_per_em as f32;
